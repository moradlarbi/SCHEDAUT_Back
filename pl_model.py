...
# Charger les données
classes_df, profs_df, matieres_df, salles_df, matieres_classes_df, matieres_profs_df = (
    charger_donnees()
)
# Mappings
...
matieres_classes = {
    ...
    for class_id, courses in matieres_classes_df.groupby("idClass")["idCourse"]
    .apply(list)
    .to_dict()
    .items()
}
matieres_profs = {
    id_to_name_teacher[teacher_id]: [
        id_to_name_course[course_id] for course_id in courses
    ]
    for teacher_id, courses in matieres_profs_df.groupby("idTeacher")["idCourse"]
    .apply(list)
    .to_dict()
    .items()
}
...
jours_ouvrables = pd.date_range(start="2024-09-01", end="2025-01-31", freq="B")
creneaux_par_jour = [
    (jour, creneau) for jour in jours_ouvrables for creneau in creneaux.keys()
]
    ...
    for jour, creneau in creneaux_par_jour:
        emploi_du_temps += (
            lpSum(
                ...
                for salle in salles
                if matiere in matieres_classes.get(classe, [])
            )
            <= 1
        )
...
for (classe, matiere, jour, creneau, prof, salle), variable in X.items():
    if variable.varValue == 1:
        emploi_du_temps_resultat.append(
            [classe, matiere, jour.strftime("%Y-%m-%d"), creneau, prof, salle]
        )
emploi_du_temps_df = pd.DataFrame(
    emploi_du_temps_resultat,
    columns=["Classe", "Matière", "Date", "Créneau", "Professeur", "Salle"],
)
# Gestion des heures
...
emploi_du_temps_df["idTeacher"] = emploi_du_temps_df["Professeur"].map(
    name_to_id_teacher
)
emploi_du_temps_df["idClassRoom"] = emploi_du_temps_df["Salle"].map(
    name_to_id_classroom
)
# Vérifier si des mappings sont manquants
if (
    emploi_du_temps_df[["idClass", "idCourse", "idTeacher", "idClassRoom"]]
    .isnull()
    .any()
    .any()
):
...
event_df = emploi_du_temps_df[
    ["startTime", "endTime", "idClass", "idCourse", "idTeacher", "idClassRoom"]
]
