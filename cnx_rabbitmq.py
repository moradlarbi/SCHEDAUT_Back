...
def callback(ch, method, properties, body):
    ...
    print(" [x] Executing pl_model.py...")

    # Execute the test_cnx.py script
    try:
        result = subprocess.run(
            ["python", "pl_model.py"], capture_output=True, text=True
        )
        if result.returncode == 0:
            ...
            print(
                " [x] Script Error Output:\n", result.stderr
            )  # Print the error output if the script fails
...
def main():
    try:
        ...
        print(
            f" [*] Waiting for messages from queue '{QUEUE_NAME}'. To exit, press CTRL+C"
        )
